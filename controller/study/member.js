...
function getMemberList(req, res, next) {
    new Promise((resolve, reject)=>{
        ...
        model.getMemberList(study_id, resolve, reject);
    }).
    then((data)=>{
        result.send(200, `${req.params.study_id}번 스터디 멤버 조회가 완료되었습니다`, data, res);
    }).
    catch((error)=>{
        next(error);
    });
...
function removeMemberFromStudy(req, res, next) {
    new Promise((resolve, reject)=>{
        ...
        model.removeMemberFromStudy(member_id, resolve, reject);
    }).
    then((data)=>{
