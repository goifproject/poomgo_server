...
function makeNewNotice(req, res, next) {
    new Promise((resolve, reject)=>{
        ...
        model.makeNewNotice(study_id, dataObj, resolve, reject);
    }).
    then((data)=>{
        result.send(200, `${req.params.study_id}번 스터디에 공지 생성이 완료되었습니다`, {}, res);
    }).
    catch((error)=>{
        next(error);
    });
...
function getNoticeInfo(req, res, next) {
    new Promise((resolve, reject)=>{
        ...
        model.getNoticeInfo(notice_id, resolve, reject);
    }).
    then((data)=>{
        result.send(200, `${req.params.study_id}번 스터디 ${req.params.notice_id}공지 조회가 완료되었습니다`, data, res);
    }).
    catch((error)=>{
        next(error);
    });
...
function getNoticeInfoList(req, res, next) {
    new Promise((resolve, reject)=>{
        ...
        let study_id = req.params.study_id;
        model.getNoticeInfoList(study_id, resolve, reject);
    }).
    then((data)=>{
        result.send(200, `${req.params.study_id}번 스터디 공지 전체 조회가 완료되었습니다`, data, res);
    }).
    catch((error)=>{
        next(error);
    });
...
function changeNoticeInfo(req, res, next) {
    new Promise((resolve, reject)=>{
        ...
        // TODO Datetime 클라이언트랑 협의 필요함
        model.changeNoticeInfo(notice_id, dataObj, resolve, reject);
    }).
    then((data)=>{
        result.send(200, `${req.params.study_id}번 스터디 ${req.params.notice_id} 공지 업데이트가 완료되었습니다`, {}, res);
    }).
    catch((error)=>{
        next(error);
    });
...
function removeNotice(req, res, next) {
    new Promise((resolve, reject)=>{
        ...
        let study_id = req.params.study_id;
        let notice_id = req.params.notice_id;
        model.removeNotice(notice_id, resolve, reject);
    }).
    then((data)=>{
        result.send(200, `${req.params.study_id}번 스터디 ${req.params.notice_id}공지 삭제가 완료되었습니다`, {}, res);
    }).
    catch((error)=>{
        next(error);
    });
